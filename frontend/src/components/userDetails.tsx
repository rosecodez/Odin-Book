  ...
  useEffect(() => {
    const getUserDetails = async () => {
      try {
        ...
        const data: User & { isFollowing: boolean } = await response.json();
        setUser(data);
      ...
      <div className="flex flex-col sm:flex-row items-center sm:gap-6 mt-6">
        <div className=" w-[150px] h-[160px]">
          <img
            ...
            className="outline outline-offset-2 outline-gray-500 rounded-full w-[150px] h-[160px]"
          />
        </div>
        ...
        <div className="h-[160px]">
          ...
          {loggedInUser && loggedInUser.username !== username && (
            <button
              className="mt-4 bg-blue-500 hover:bg-indigo-600 text-white font-bold mb-2 py-2 rounded focus:outline-none focus:shadow-outline self-center w-[150px]"
              onClick={followUser}
            >
